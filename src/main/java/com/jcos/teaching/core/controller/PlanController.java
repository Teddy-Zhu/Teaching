	...
	public boolean exportAllClassExcel(HttpServletRequest request, Model model, HttpServletResponse response) {
		...
		if (plans.size() > 0) {
			...
			for (int i = 0, len = plans.size(); i < len; i++) {
				...
				excel.setCellDouble(curLine, 3, bookPrice);
				excel.setCellDouble(curLine, 4, curPlan.getIntstudcount().doubleValue());
				excel.setCellDouble(curLine, 5, curPlan.getIntteaccount().doubleValue());
				excel.setCellDouble(curLine, 6, disCount);
				excel.setCellFormula(curLine, 7, "PRODUCT(D" + (curLine + 1) + ",E" + (curLine + 1) + ")");
				excel.setCellFormula(curLine, 8, "PRODUCT(D" + (curLine + 1) + ",F" + (curLine + 1) + ")");
				excel.setCellFormula(curLine, 9, "PRODUCT(G" + (curLine + 1) + ",H" + (curLine + 1) + ")/10");
				excel.setCellFormula(curLine, 10, "PRODUCT(G" + (curLine + 1) + ",I" + (curLine + 1) + ")/10");
