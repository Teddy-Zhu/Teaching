...
function DrawAmChart3() {
	...
	valueAxis2.position = "right"; // this line makes the axis to appear on the
									// right
	...
	valueAxis3.offset = 50; // this line makes the axis to appear detached from
							// plot area
	...
	graph2.valueAxis = valueAxis2; // we have to indicate which value axis
									// should be used
	...
	graph3.valueAxis = valueAxis3; // we have to indicate which value axis
									// should be used
...
function DrawChartistChart1() {
	Chartist.Line('#chartist-1', {
		...
		series : [ [ 12, 9, 7, 8, 5, 5, 17, 11, 12, 6, 3, 9 ], [ 2, 1, 3.5, 7, 3, 6, 2, 9, 1, 21, 15, 1 ], [ 1, 3, 4, 5, 6, 1, 15, 3, 9, 11, 18, 14 ] ]
	});
}
...
function DrawChartistChart3() {
	Chartist.Line('#chartist-3', {
		...
		series : [ [ 1, 2, 3, 1, -2, 0, 1, 0, 3, -1, 1 ], [ -2, -1, -2, -1, -2.5, -1, -2, -1, -2, 2, -2 ], [ 0, 0, 0, 1, 2, 2.5, 2, 1, 4, -3, 1 ], [ 2.5, 2, 1, 0.5, 1, 0.5, -1, -2.5, -1, 2, 1 ] ]
	}, {
		high : 3,
		low : -3,
		showArea : true,
...
function DrawChartistChart5() {
	var data = {
		...
		series : [ [ 1, 2, 4, 8, 6, -2, -1, -4, -6, -2, 3, 6, 1, -4, 2, 7, -1, 3 ] ]
	};
	var options = {
		high : 10,
		low : -10,
...
function TinyMCEStart(elem, mode) {
	var plugins = [];
	if (mode == 'extreme') {
		plugins = [ "advlist anchor autolink autoresize autosave bbcode charmap code contextmenu directionality ", "emoticons fullpage fullscreen hr image insertdatetime layer legacyoutput",
				"link lists media nonbreaking noneditable pagebreak paste preview print save searchreplace", "tabfocus table template textcolor visualblocks visualchars wordcount" ]
	}
	tinymce
			.init({
				selector : elem,
...
function DrawAllCharts() {
	// Chart 1
	var chart1_data = [ [ 'Smartphones', 'PC', 'Notebooks', 'Monitors', 'Routers', 'Switches' ], [ '01.01.2014', 1234, 2342, 344, 232, 131 ], [ '02.01.2014', 1254, 232, 314, 232, 331 ],
			[ '03.01.2014', 2234, 342, 298, 232, 665 ], [ '04.01.2014', 2234, 42, 559, 232, 321 ], [ '05.01.2014', 1999, 82, 116, 232, 334 ], [ '06.01.2014', 1634, 834, 884, 232, 191 ],
			[ '07.01.2014', 321, 342, 383, 232, 556 ], [ '08.01.2014', 845, 112, 499, 232, 731 ] ];
	var chart1_options = {
	...
	// Chart 2
	var chart2_data = [ [ 'Height', 'Width' ], [ 'Samsung', 74.5 ], [ 'Apple', 31.24 ], [ 'LG', 12.10 ], [ 'Huawei', 11.14 ], [ 'Sony', 8.3 ], [ 'Nokia', 7.4 ], [ 'Blackberry', 6.8 ],
	...
	// Chart 4
	var chart4_data = [ [ 'ID', 'Life Expectancy', 'Fertility Rate', 'Region', 'Population' ], [ 'CAN', 80.66, 1.67, 'North America', 33739900 ], [ 'DEU', 79.84, 1.36, 'Europe', 81902307 ],
			[ 'DNK', 78.6, 1.84, 'Europe', 5523095 ], [ 'EGY', 72.73, 2.78, 'Middle East', 79716203 ], [ 'GBR', 80.05, 2, 'Europe', 61801570 ], [ 'IRN', 72.49, 1.7, 'Middle East', 73137148 ],
			[ 'IRQ', 68.09, 4.77, 'Middle East', 31090763 ], [ 'ISR', 81.55, 2.96, 'Middle East', 7485600 ], [ 'RUS', 68.6, 1.54, 'Europe', 141850000 ],
	...
	// Chart 6
	var chart6_data = [ [ 'Year', 'Sales', 'Expenses' ], [ '2004', 1000, 400 ], [ '2005', 1170, 460 ], [ '2006', 660, 1120 ], [ '2007', 1030, 540 ], [ '2008', 2080, 740 ], [ '2009', 1949, 690 ],
			[ '2010', 2334, 820 ] ];
	var chart6_options = {
		backgroundColor : '#fcfcfc',
...
function MorrisDashboard() {
	...
	Morris.Bar({
		...
		data : [ {
			...
			x : 'New optical channels',
			y : 22359
		}, {
			x : 'Load balance system',
			y : 33950
		} ],
		xkey : 'x',
		ykeys : [ 'y' ],
	...
	Morris.Donut({
		...
		data : [ {
			...
			label : 'Improved power equipment',
			value : 59411
		}, {
			label : 'New ticket system',
			value : 14906
		}, {
			label : 'Storage area network',
			value : 250000
		}, {
			label : 'New optical channels',
			value : 22359
		}, {
			label : 'Load balance system',
			value : 33950
		} ],
		colors : [ '#CCC', '#DDD', '#BBB' ]
	});
...
function LoadTestMap() {
	$.getJSON("http://www.telize.com/geoip?callback=?", function(json) {
		...
		var osmap1 = new OpenLayers.Layer.OSM("OpenStreetMap");// создание слоя
																// карты
...
function CreateStockPage() {
	...
	$("#submit").click(
			function() {
				...
				var historicalQ = yqlURL + "select%20*%20from%20yahoo.finance.historicaldata%20where%20symbol%20%3D%20%22" + symbol + "%22%20and%20startDate%20%3D%20%22" + startDate
						+ "%22%20and%20endDate%20%3D%20%22" + endDate + "%22" + dataFormat;
				$(function() {
					$.getJSON(realtimeQ, function(json) {// YQL Request
						$('#symbol').text(json.query.results.quote.Name);// Assign
																			// quote.Param
																			// to
																			// span
																			// tag
				...
				$(function() {
					$.getJSON(historicalQ, function(json) {
						$.each(json.query.results.quote, function(i, quote) {// loop
																				// results.quote
																				// object
							$("#date").append('<span>' + quote.Date + '</span');// create
																				// span
																				// for
																				// each
																				// record
						});
						$.each(json.query.results.quote, function(i, quote) { // new
																				// each
																				// statement
																				// is
																				// needed
	...
	var calendar = $('#calendar').fullCalendar(
			{
				...
				select : function(start, end, allDay) {
					var form = $('<form id="event_form">' + '<div class="form-group has-success has-feedback">' + '<label">Event name</label>' + '<div>'
							+ '<input type="text" id="newevent_name" class="form-control" placeholder="Name of event">' + '</div>' + '<label>Description</label>' + '<div>'
							+ '<textarea rows="3" id="newevent_desc" class="form-control" placeholder="Description"></textarea>' + '</div>' + '</div>' + '</form>');
					var buttons = $('<button id="event_cancel" type="cancel" class="btn btn-default btn-label-left">' + '<span><i class="fa fa-clock-o txt-danger"></i></span>' + 'Cancel'
							+ '</button>' + '<button type="submit" id="event_submit" class="btn btn-primary btn-label-left pull-right">' + '<span><i class="fa fa-clock-o"></i></span>' + 'Add'
							+ '</button>');
				...
				eventClick : function(calEvent, jsEvent, view) {
					var form = $('<form id="event_form">' + '<div class="form-group has-success has-feedback">' + '<label">Event name</label>' + '<div>'
							+ '<input type="text" id="newevent_name" value="' + calEvent.title + '" class="form-control" placeholder="Name of event">' + '</div>' + '<label>Description</label>'
							+ '<div>' + '<textarea rows="3" id="newevent_desc" class="form-control" placeholder="Description">' + calEvent.description + '</textarea>' + '</div>' + '</div>'
							+ '</form>');
					var buttons = $('<button id="event_cancel" type="cancel" class="btn btn-default btn-label-left">' + '<span><i class="fa fa-clock-o txt-danger"></i></span>' + 'Cancel'
							+ '</button>' + '<button id="event_delete" type="cancel" class="btn btn-danger btn-label-left">' + '<span><i class="fa fa-clock-o txt-danger"></i></span>' + 'Delete'
							+ '</button>' + '<button type="submit" id="event_change" class="btn btn-primary btn-label-left pull-right">' + '<span><i class="fa fa-clock-o"></i></span>'
							+ 'Save changes' + '</button>');
